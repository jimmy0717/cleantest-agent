...
def detect_ambiguous_type(root_node) -> bool:
    ...
    Note: The original CleanTest paper flags ALL methods with generic markers
    (<E>, <T>, <K>, <V>, <N>, <?>). Our implementation is more precise:
    we only flag generics without 'extends' bounds, as bounded generics
    provide sufficient type information for test generation.
    """
    stack = [root_node]
    while stack:
        node = stack.pop()
