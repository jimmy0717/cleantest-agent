...
CleanTest-Agent fulfills all of these requirements: it applies software requirements analysis and system design principles to a real-world problem, implements a model-driven approach with Agent Skills, provides comprehensive experimental evaluation, and includes 38 test cases with GitHub Actions CI/CD.
...
The common thread across all four generations is that \textbf{the quality of training data fundamentally constrains model performance}. If 43.52\% of the training data is noisy, even the most sophisticated model architecture cannot fully compensate.
...
Allamanis~\cite{allamanis2019adverse} conducted a seminal study on the adverse effects of code duplication in machine learning models of code. Using the popular \textit{java-small}, \textit{java-med}, and \textit{java-large} benchmarks, Allamanis showed that near-duplicate files between training and test sets inflate evaluation metrics by 10--100\%, depending on the task. This work led to the standard practice of deduplication in dataset construction --- a practice we also apply as the first step in our pipeline.
...
\midrule
FR1 & \texttt{src/parser\_utils.py} & \texttt{test\_syntax\_filter.py} (14 cases) \\
...
Our experiments (Section~\ref{sec:evaluation}) demonstrate that the model-driven approach achieves higher F1 scores than pure-LLM approaches at lower cost. We argue this reflects three fundamental properties of the data cleaning task:

\begin{enumerate}
    \item \textbf{Pattern-Matching Subtask}: With 21,954 annotation patterns to match, no LLM can reliably enumerate them all from a prompt context. A deterministic dictionary is provably superior.

    \item \textbf{AST Subtask}: Syntactic structure (e.g., empty catch blocks) is most precisely identified by an actual parser like tree-sitter. LLMs only approximate parsing and may miss subtle structural features.

    \item \textbf{Semantic Subtask}: For some borderline cases (e.g., is a test indirectly testing the focal method through a wrapper?), only an LLM can apply common-sense reasoning. But this only applies to a small fraction ($\sim$12.7\%) of samples.
\end{enumerate}

The model-driven approach uses each tool for the subtask it is best at. The pure-LLM approach forces the LLM to do all subtasks, including those it is poorly suited for.
...
\midrule
\texttt{test\_syntax\_filter.py} & 14 & All 7 noise types + annotations \\
...
$-$ LLM enhancement & 1.000 & 1.000 & 1.000 & +0.035 \\
...
\begin{enumerate}
    ...
    \item \textbf{The LLM enhancement slightly reduces F1 in the ablation} because the ``full system'' uses LLM-simulated labels while the ``$-$ LLM'' uses pure rule labels as ground truth. In practice, the LLM enhancement corrects $\sim$5\% of rule false positives at a small cost.
\end{enumerate}
